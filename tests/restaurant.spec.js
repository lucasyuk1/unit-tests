...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    const m5 = createMenu();
    m5.order('coxinha');
    expect(m5.consumption[0]).toBe('coxinha');
    ...
    m6.order('sopa');
    m6.order('sashimi');
    expect(m6.consumption).toEqual(["coxinha", "agua", "sopa", "sashimi"]);
    ...
    m7.order('agua');
    m7.order('coxinha');
    expect(m7.consumption).toEqual(["coxinha", "agua", "coxinha"]);
    ...
    // ```
    const menu = { food: { coxinha: 3.5, sopa: 12 }, drink: { agua: 1.9, cerveja: 4.9} }
    const m8 = createMenu(menu);
    m8.order('coxinha');
    m8.order('agua');
    m8.order('coxinha');
    expect(m8.pay()).toBe(8.9);
