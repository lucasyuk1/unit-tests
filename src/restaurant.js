...
const createMenu = (obj) => {
  const rest = {
    ...
    consumption: [],
    order: (args) => {
      rest.consumption.push(args);
    },
    pay: () => {
      let cont = 0;
      const iMenu = rest.fetchMenu();
      const iFood = iMenu.food;
      const iDrink = iMenu.drink;
      const iCons = rest.consumption;
      for (let index of iCons) {
        if (iFood[index])cont += iFood[index];
        if (iDrink[index])cont += iDrink[index];
      }
      return cont;
    },
  };
